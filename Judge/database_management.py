...
class manage_database():
	...
	def initialize_database():
		...
		try:
			c.execute("create table if not exists verdict(run_id integer , client_id varchar2(10), verdict varchar2(6), language varchar2(10) , message varchar2(30), p_code varchar2(10), time_stamp varchar2(10), source varchar2(30))")

		except Exception as error:
			print("[ DB ] Table creation error: "+str(error))
	...
	def insert_record(run_id, client_id, verdict,language, message, p_code, time_stamp, source_code):
		with manage_database.conn:
			...
			try:
				...
				print("[ ERROR ] Insertion error: "+str(error))
